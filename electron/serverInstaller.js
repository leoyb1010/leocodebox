...
/**
 * Installs the versioned local server runtime used by leocodebox.
 *
 * Server bundles are cached under:
 *   ~/.leocodebox/server/<version>/dist-server/server/index.js
 */
...
export class ServerInstaller {
  constructor({
    ...
    arch = process.arch,
    installRoot = process.env.LEOCODEBOX_SERVER_DIR || process.env.CLOUDCLI_SERVER_DIR || DEFAULT_INSTALL_ROOT,
    bundleBaseUrl = process.env.LEOCODEBOX_SERVER_BUNDLE_URL || process.env.CLOUDCLI_SERVER_BUNDLE_URL || DEFAULT_BUNDLE_BASE_URL,
    bundleReleaseTag = process.env.LEOCODEBOX_SERVER_BUNDLE_RELEASE_TAG || process.env.CLOUDCLI_SERVER_BUNDLE_RELEASE_TAG || '',
    onLog,
  } = {}) {
  ...
  getBundleName() {
    return `leocodebox-local-server-${this.version}-${this.platform}-${this.arch}.tar.gz`;
  }
