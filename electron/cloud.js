...
export class CloudController {
  ...
  getEnvironmentUrl(environment) {
    return environment.access_url || `https://${environment.subdomain}.leocodebox.local`;
  }
  ...
  async cloudApi(pathname, options = {}) {
    if (!this.cloudAccount?.apiKey) {
      throw new Error('Connect your leocodebox account first.');
    }
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error(`leocodebox API request timed out after ${Math.round(CLOUD_API_TIMEOUT_MS / 1000)} seconds.`);
      }
      throw error;
    } finally {
    ...
    if (!response.ok) {
      ...
      throw new Error(body.error || `leocodebox API request failed: ${response.status}`);
    }
  ...
  buildConnectUrl() {
    ...
    connectUrl.searchParams.set('app_surface', 'leocodebox_desktop');
