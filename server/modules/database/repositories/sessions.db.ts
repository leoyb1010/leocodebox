...
export const sessionsDb = {
  ...
  /** Returns non-archived session counts grouped by provider for one project path. */
  getSessionProviderCountsByProjectPath(projectPath: string): Record<string, number> {
    const db = getConnection();
    const normalizedProjectPath = normalizeProjectPath(projectPath);
    const rows = db
      .prepare(
      ...
      .all(normalizedProjectPath) as Array<{ provider: string; count: number }>;

    const counts: Record<string, number> = {};
    for (const row of rows) {
      counts[row.provider] = Number(row.count ?? 0);
    }
    return counts;
  },
