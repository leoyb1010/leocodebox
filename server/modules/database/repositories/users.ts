...
export const userDb = {
  ...
  getOrCreateLocalUser(): UserPublicRow {
    const db = getConnection();
    const existing = this.getFirstUser();
    if (existing) {
      this.completeOnboarding(existing.id);
      return this.getUserById(existing.id) || existing;
    }

    try {
      const result = db
        .prepare(
          'INSERT INTO users (username, password_hash, has_completed_onboarding) VALUES (?, ?, 1)'
        )
        .run('local-agent', 'leocodebox-local-only');
      const user = this.getUserById(Number(result.lastInsertRowid));
      if (user) return user;
    } catch (error) {
      const racedUser = this.getFirstUser();
      if (racedUser) {
        this.completeOnboarding(racedUser.id);
        return this.getUserById(racedUser.id) || racedUser;
      }
      throw error;
    }

    throw new Error('Failed to create local desktop user');
  },
