...
test('providerSkillsService manages OpenCode user skills', { concurrency: false }, async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'llm-skills-opencode-write-'));
  const restoreHomeDir = patchHomeDir(tempRoot);
  try {
    const added = await providerSkillsService.addProviderSkills('opencode', {
      entries: [
        {
    ...
    assert.equal(added[0]?.name, 'opencode-global');
    assert.match(added[0]?.sourcePath ?? '', /\.config[\\/]opencode[\\/]skills/);

    const removed = await providerSkillsService.removeProviderSkill('opencode', {
      directoryName: 'opencode-global-dir',
    });
