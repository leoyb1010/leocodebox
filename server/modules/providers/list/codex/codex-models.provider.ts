...
// Fallback catalog used only when ~/.codex/models_cache.json cannot be read.
// Kept current with the installed Codex CLI's frontier models so a cold read
// still surfaces the latest generation instead of a stale one.
export const CODEX_FALLBACK_MODELS: ProviderModelsDefinition = {
  OPTIONS: [
    { value: 'gpt-5.6-sol', label: 'gpt-5.6-sol', effort: CODEX_EFFORT },
    { value: 'gpt-5.6-terra', label: 'gpt-5.6-terra', effort: CODEX_EFFORT },
    { value: 'gpt-5.6-luna', label: 'gpt-5.6-luna', effort: CODEX_EFFORT },
    { value: 'gpt-5.5', label: 'gpt-5.5', effort: CODEX_EFFORT },
    { value: 'gpt-5.4', label: 'gpt-5.4', effort: CODEX_EFFORT },
  ],
  DEFAULT: 'gpt-5.6-sol',
};
...
export class CodexProviderModels implements IProviderModels {
  ...
  /**
   * Fingerprint of the Codex CLI model cache (size + mtime). When the Codex CLI
   * updates and rewrites models_cache.json, this changes and forces a refetch,
   * so new frontier models (e.g. the 5.6 generation) appear without a manual
   * cache clear or waiting out the app-level TTL.
   */
  async getCacheFingerprint(): Promise<string | null> {
    try {
      const info = await stat(CODEX_MODELS_CACHE_PATH);
