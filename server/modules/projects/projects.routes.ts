...
router.post('/clone-progress', async (req, res) => {
  ...
  try {
    ...
    const workspacePath = readQueryStringValue(body.path);
    const githubUrl = readQueryStringValue(body.githubUrl);
    const githubTokenId = readOptionalNumericQueryValue(body.githubTokenId);
    const newGithubToken = readQueryStringValue(body.newGithubToken) || null;
