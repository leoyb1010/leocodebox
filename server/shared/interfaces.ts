...
export interface IProviderModels {
  ...
  /**
   * Optional freshness fingerprint of the provider's model source (e.g. the CLI's
   * own model cache file). When the fingerprint changes, the model-cache layer
   * treats any cached catalog for this provider as stale and refetches — so an
   * upgraded CLI surfaces new models without waiting for the cache TTL to expire.
   * Return `null` when no external source exists or it cannot be read.
   */
  getCacheFingerprint?(): Promise<string | null>;
}
