...
function migrateLegacyUserDatabase() {
  if (fs.existsSync(DEFAULT_DATABASE_PATH) || !fs.existsSync(LEGACY_DATABASE_PATH)) return;
  try {
    fs.mkdirSync(path.dirname(DEFAULT_DATABASE_PATH), { recursive: true });
    fs.copyFileSync(LEGACY_DATABASE_PATH, DEFAULT_DATABASE_PATH);
    for (const suffix of ['-wal', '-shm']) {
      const legacySidecar = LEGACY_DATABASE_PATH + suffix;
      if (fs.existsSync(legacySidecar)) {
        fs.copyFileSync(legacySidecar, DEFAULT_DATABASE_PATH + suffix);
      }
    }
  } catch (error) {
