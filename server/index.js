...
import leocodeboxRoutes from './routes/leocodebox.js';
...
app.get('/api/projects/:projectId/file', authenticateToken, async (req, res) => {
    try {
        ...
        if (!realCheck.valid) {
            return res.status(403).json({ error: realCheck.error });
        }
...
app.get('/api/projects/:projectId/files/content', authenticateToken, async (req, res) => {
    try {
        ...
        const mimeType = mime.lookup(realCheck.realPath) || 'application/octet-stream';
...
/**
 * Symlink-safe containment check. The string-prefix guard above can be defeated
 * by a symlink inside the project that points outside it, so callers that touch
 * the filesystem must additionally confirm the *canonical* path stays within the
 * canonical project root. Canonicalizes the deepest existing ancestor so it also
 * works for not-yet-created targets (allowMissing).
 */
async function assertRealPathWithinRoot(projectRoot, resolvedPath, { allowMissing = false } = {}) {
    let realRoot;
    try {
        realRoot = await fsPromises.realpath(path.resolve(projectRoot));
    } catch {
...
app.put('/api/projects/:projectId/files/rename', authenticateToken, async (req, res) => {
    try {
        ...
        if (!oldRealCheck.valid) {
            return res.status(403).json({ error: oldRealCheck.error });
        }
        ...
        const newRealCheck = await assertRealPathWithinRoot(projectRoot, resolvedNewPath, { allowMissing: true });
        if (!newRealCheck.valid) {
            return res.status(403).json({ error: newRealCheck.error });
        }
...
app.delete('/api/projects/:projectId/files', authenticateToken, async (req, res) => {
    try {
        ...
        // Canonicalize the PARENT directory so a symlinked ancestor cannot make the
        // deletion escape the project root. The entry itself is deleted by its
        // original path so we remove a symlink rather than following it.
        const parentRealCheck = await assertRealPathWithinRoot(projectRoot, path.dirname(resolvedPath));
        if (!parentRealCheck.valid) {
            return res.status(403).json({ error: parentRealCheck.error });
        }

        // Check if path exists and get stats (lstat: do not follow a final symlink)
        let stats;
        try {
    ...
    uploadMiddleware.array('files', MAX_FILE_UPLOAD_COUNT)(req, res, async (err) => {
        ...
        try {
            ...
            if (!targetDir || targetDir === '.' || targetDir === './') {
                ...
                if (!targetRealCheck.valid) {
                    return res.status(403).json({ error: targetRealCheck.error });
                }
                resolvedTargetDir = targetRealCheck.realPath;
                console.log('[DEBUG] Resolved target dir:', resolvedTargetDir);
            }

            // Ensure target directory exists (canonicalized above, so mkdir cannot
            // escape the project root via a symlinked ancestor).
            try {
            ...
            for (let i = 0; i < req.files.length; i++) {
                ...
                const destRealCheck = await assertRealPathWithinRoot(projectRoot, destPath, { allowMissing: true });
                if (!destRealCheck.valid) {
                    await fsPromises.unlink(file.path).catch(() => {});
                    continue;
                }
                ...
                const parentDir = path.dirname(safeDestPath);
                try {
                    await fsPromises.access(parentDir);
                } catch {
                    await fsPromises.mkdir(parentDir, { recursive: true });
