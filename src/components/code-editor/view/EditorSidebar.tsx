    ...
    <div ref={containerRef} className={`flex h-full min-w-0 ${editorExpanded ? 'flex-1' : ''}`}>
      {!editorExpanded && (
        <div
          ...
          className="group relative w-1 flex-shrink-0 cursor-col-resize bg-muted transition-colors hover:bg-info dark:bg-muted dark:hover:bg-info"
          title="Drag to resize"
        >
          <div className="absolute inset-y-0 left-1/2 w-1 -translate-x-1/2 bg-info opacity-0 transition-opacity group-hover:opacity-100 dark:bg-info" />
        </div>
      )}

      <div
        className={`h-full overflow-hidden border-l border-border dark:border-border ${useFlexLayout ? 'min-w-0 flex-1' : `min-w-[ flex-shrink-0${MIN_EDITOR_WIDTH}px]`}`}
